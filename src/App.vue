...
export default {
	...
	computed: {
		 ...mapGetters({
			listMenu    : getter.GET_ROLE_MENUS
		}),
		navPos() {
	...
	methods: {
		...
		parseMenuRoles() {
			let menu_roles = localStorage.getItem('menu_roles');
			if(menu_roles) {
				this.$store.commit(mutation.SET_ROLE_MENUS, JSON.parse(menu_roles))
			}
		}
	},
	components: {
	...
	created() {
		...
		//this.$store.dispatch(action.FETCH_MENU);
	},
	mounted() {
		this.resizeOpenNav()
