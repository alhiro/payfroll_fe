                ...
                <el-col :lg="8">
                    <el-form-item label="Country" prop="countryId">
                      <el-select v-model="form.countryId" filterable placeholder="Select Country">
                        <el-option v-for="item in country" :key="item.countryId" :label="item.countryName" :value="item.countryId"></el-option>
                      </el-select>
                    </el-form-item>
                </el-col>
            </el-row>
...
export default {
  ...
  methods: {
    onSubmit(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        ...
        if (valid) {
          ...
          this.$store.dispatch(action.CREATE_DATA_USER_HOLDING, obj)
        } else {
          return false 
        }
      }) 
    ...
    Cancel() {
      this.$router.replace('/list-user-holding')
    },
    isNumber(evt) {
  ...
  computed: {
    ...mapGetters({
      ...
      userRoles: getter.GET_USER_ROLES_HOLDING,
      country: getter.GET_COUNTRY,
      button: getter.GET_BUTTON
    })
  },
  mounted() {
    this.$store.dispatch(action.USER_ROLES_HOLDING)
    this.$store.dispatch(action.COUNTRY_LIST)
  }
} 
</script>
