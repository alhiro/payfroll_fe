        ...
        <el-row :gutter="10">
            <el-col :span="24">
                <el-table :default-sort="{prop: 'id', order: 'ascending'}" :data="listData" style="width: 100%">
                    ...
                    <el-table-column align="left" prop="name" label="Company Name"></el-table-column>
                    <el-table-column align="left" prop="holdingName" label="Holding Name"></el-table-column>
                    <el-table-column align="left" prop="status" label="Status">
                        <template slot-scope="scope">
                            <el-tag :type="scope.row.status === 0 ? 'primary' : 'success'" disable-transitions>{{scope.row.status === 0 ? 'Inactive' : 'Active&nbsp;&nbsp;'}}</el-tag>
                        </template>
                    </el-table-column>
...
export default {
    ...
    computed: {
        ...mapGetters({
            ...
            listData: getter.GET_LIST_COMPANY,
            paginate: getter.GET_PAGINATE_COMPANY,
            total: getter.GET_COMPANY_TOTAL
        }),
        paginator() {
    ...
    methods: {
        handlePageChange(page) {
            ...
            this.$store.dispatch(action.LIST_COMPANY, {
                page: page,
            });
        },
        Create() {
        ...
        Edit(e) {
            this.$store.dispatch(action.SELECT_HOLDINGS)
            this.$store.dispatch(action.DETAIL_COMPANY, e.id);
        },
        Detail(e) {
            this.$store.dispatch(action.LIST_ASSIGNED, {
                page: 1,
                id: e.id
            });
            this.$store.dispatch(action.COMPANY_DETAIL, e.id);
        },
        Delete(e) {
            Swal.fire({
                title: "Are you sure?",
                ...
                if (result.value) {
                    this.$store.dispatch(action.DELETE_COMPANY, e.id);
                }
            });
        }
    },
    async mounted() {
        await this.$store.dispatch(action.LIST_COMPANY, this.payload);
