...
export default {
  async [action.DETAIL_TAX_REGULATION_MASTER]({ commit, dispatch }, payload) {
    try {
      let token = localStorage.getItem("token");
      let { data } = await localAxios.get(
        `gco/master/tax/regulations/2`,
        {
          headers: {
      ...
      commit(mutation.SET_TAX_REGULATION_MASTER_DETAIL, data.data);
      router.push('/reset-tax-regulation')
    } catch (err) {
